...
@Injectable()
export class Brakes {
  constructor(private readonly brakesFactory: BrakesFactory, private readonly brakesRegistry: BrakesRegistry) {}
  ...
  slave(nameOrFallback: string | Fallback, fallbackOrRef: Fallback | Function, func?: Function): Function {
    const [name, fallback, ref] =
      typeof nameOrFallback === 'string' && func
        ...
        : ['BRAKES_DEFAULT_NAME', nameOrFallback as Fallback, fallbackOrRef as Function]
    ...
    let circuit
    if (fallback) {
      circuit = brakes.subCircuit(ref, fallback.fallback, fallback.config ? fallback.config() : {})
    } else {
      circuit = brakes.subCircuit(ref)
    }
